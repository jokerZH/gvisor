...
func (ke *KernelIPTEntry) MarshalBytes(dst []byte) {
	...
	marshalledUntil := ke.Entry.SizeBytes()
	if marshalledUntil < len(dst) {
		ke.Elems.MarshalBytes(dst[marshalledUntil:])
	}
}
...
func (ke *KernelIPTEntry) UnmarshalBytes(src []byte) {
	...
	marshalledUntil := ke.Entry.SizeBytes()
	if marshalledUntil < len(src) {
		ke.Elems.UnmarshalBytes(src[marshalledUntil:])
	}
}
...
func (ke *KernelIPTEntry) CopyIn(task marshal.Task, addr usermem.Addr) (int, error) {
...
func (ke *KernelIPTEntry) CopyOut(task marshal.Task, addr usermem.Addr) (int, error) {
	return ke.CopyOutN(task, addr, ke.SizeBytes())
}

// CopyOutN implements marshal.Marshallable.CopyOutN.
func (ke *KernelIPTEntry) CopyOutN(task marshal.Task, addr usermem.Addr, limit int) (int, error) {
	// Type Stat doesn't have a packed layout in memory, fall back to MarshalBytes.
	buf := task.CopyScratchBuffer(limit) // escapes: okay.
	ke.MarshalBytes(buf)                 // escapes: fallback.
	return task.CopyOutBytes(addr, buf)  // escapes: okay.
}

// WriteTo implements io.WriterTo.WriteTo.
func (ke *KernelIPTEntry) WriteTo(w io.Writer) (int64, error) {
...
// KernelIPTGetEntries has been manually made marshal.Marshallable since it is
// dynamically sized.
...
func (ke *KernelIPTGetEntries) MarshalBytes(dst []byte) {
	...
	for i := 0; i < len(ke.Entrytable) && marshalledUntil < len(dst); i++ {
...
func (ke *KernelIPTGetEntries) UnmarshalBytes(src []byte) {
	...
	for i := 0; i < len(ke.Entrytable) && unmarshalledUntil < len(src); i++ {
